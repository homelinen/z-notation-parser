...
the structures are all created together.
...
\texttt{print\_type} prints the type by type\_print method.

\subsection{pair.c}

The Pair struct has two child elements: left and right. Left and right.
...
The print\_pair method prints the two
values, seperated by commas, and wraps them in round brackets.

pair\_equality compares each side of the pair with the corresponding side of the second pair. Will not work for different ordered pairs.
...
The print\_set method takes a set and prints every element, seperated by
a comma and surrounds with curly braces. Nested structures are printed using the print\_type function.
